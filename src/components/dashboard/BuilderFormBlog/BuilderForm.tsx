...
import { fetchComponetBlogs } from "@/services/builder.service";
...
const BuilderFormBlog = ({ pageId, pageType }: BuilderFormProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchComponetBlogs();
  ...
  const getComponent = useCallback((name: string) => {
    if (!componentCache.current[name]) {
      componentCache.current[name] = React.lazy(
        () => import(`../../../components/builderBlog/${name}/${name}`)
      );
    }
...
export default BuilderFormBlog;
