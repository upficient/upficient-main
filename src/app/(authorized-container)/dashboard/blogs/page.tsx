...
export default function Blogs() {
  ...
  const fetchPages = async () => {
    setLoading(true);
    try {
      const data = await getAllPages("blog");
      setData(data);
    } catch (error) {
      console.error("Error fetching plogs:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="main-user-wrapper">
      ...
      >
        <Link href="blogs/add">
          <button className="btn-dash">Add New Blog</button>
        </Link>
      </div>
            ...
            <div className="listbox">
              ...
              <div className="list-content">
                {currentData.map((item) => {
                  const pageTitleField = item.fields.find(
                  ...
                  const slug =
                    item.componentType === "blog"
                      ? `clickup-guides/${slugField?.value}`
                      : slugField?.value;
                  return (
